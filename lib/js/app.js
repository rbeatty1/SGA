            ...
            //secondaryActions
                ...
                //action 1 (secondary mouseover)
                    function secondaryHighlight(e){
                        ...
                        if (props.TOTAL_SGA == 1){
                            muni.bindTooltip('<b><center>'+props.MUN_NAME+'</center>'+props.TOTAL_SGA+'</b> Municipal Smart Growth Action').openTooltip();                            
                        }
                        else{
                            muni.bindTooltip('<b><center>'+props.MUN_NAME+'</center>'+props.TOTAL_SGA+'</b> Municipal Smart Growth Actions').openTooltip();
                        }

                    }

                //action 2 (secondary mouseout)s
                    function secondaryReset(e){
                        var layer = e.target;
                        muni.resetStyle(layer);
                    }

                //action 3 (secondary on click)
                    function secondaryClick(e){
                        var layer = e.target;
                        var props = layer.feature.properties;
                        var muniTarget = props.MUN_NAME;
                        console.log("You clicked on "+muniTarget);
                    }
